...
//       {/* --- Section 1: Top Notch Facilities (The List) --- */}
...
export default function Facilities() {
  return (
    <section className="max-w-7xl mx-auto px-6 py-20">
      {/* Heading */}
      <div className="mb-10">
        <h2 className="text-4xl font-bold text-[#0f172a]">OUR FACILITIES</h2>
        <div className="mt-2 h-[3px] w-16 bg-sky-500" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <FacilityCard
          title="SWIMMING POOL"
          description="A one stop destination of aquatic talents. Our students went to domestic as well as State competition."
          icon={<Waves size={36} />}
        ...
        <FacilityCard
          title="GYM"
          description="A state of the art facility where people can train, do weight management and work on their physique."
    ...
    <div className="group perspective-[1200px] h-[260px]">
      <div className="relative h-full w-full transition-transform duration-700 preserve-3d group-hover:rotate-y-180">
        ...
        <div className="absolute inset-0 backface-hidden">
          ...
          >
            <div className="absolute inset-0 bg-blue-600/80 flex flex-col items-center justify-center text-white px-8 text-center">
              <div className="mb-4">{icon}</div>
              <h3 className="text-xl font-semibold tracking-wide">{title}</h3>
              <p className="mt-4 text-sm opacity-90">{description}</p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 rotate-y-180 backface-hidden">
          <div
            ...
            style={{ backgroundImage: `url(${image})` }}
          >
            <div className="absolute inset-0 bg-blue-700/85 flex flex-col items-center justify-center text-white px-8 text-center">
              <h3 className="text-2xl font-semibold mb-4">{title}</h3>
              <p className="text-sm max-w-md">{description}</p>

              <button className="mt-6 border border-white px-6 py-2 text-sm hover:bg-white hover:text-blue-700 transition">
                Know More
              </button>
            </div>
          </div>
